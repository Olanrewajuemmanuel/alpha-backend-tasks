...
Upload a document for a candidate. The file is expected to be a plain text file where text extraction is much easier and won't break. An `example-resume.txt` file is provided in the repository.
...
- **Text extraction** — text extraction is currently implemented as a simple `buffer.toString('utf-8')` which only works for plain text files. We assume that the files passed are plain text. A proper pipeline would use libraries for different file types.
